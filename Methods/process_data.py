...
class ProcessData:
    ...
    def get_processed_df(file_path: str, type: str = DATA_TYPE.BENIGN, shall_drop_column: bool = True):
    ...
    def get_payload_data(file_df: pd.DataFrame, column_list: list):
        for i in file_df.columns:
            if i not in column_list:
                file_df = file_df.drop(i, axis=1)
        return file_df

    def get_file_by_day(day: int, type: str = DATA_TYPE.BENIGN, shall_drop_column: bool = True):
        file_df = ProcessData.get_processed_df(FILES_PATH + FILE_NAME_LIST[day][0], type, shall_drop_column)
        for i in FILE_NAME_LIST[day][1:]:
            file_df = pd.concat([file_df, ProcessData.get_processed_df(FILES_PATH + i, type, shall_drop_column)], axis=0)
        return file_df
